...
\academicsReport{实验二}{2026 年 3 月 23 日}
...
\paragraph{题目描述} {
    手机系统中，蓝牙适配器只支持一个。因此在整个软件系统生命期内，只允许创建一个对象，不允许创建多个这样的对象（创建多个像蓝牙适配器对象是没有意义的）。如何确保一个类的对象只允许创建一个呢？通常程序员是通过 \mil[java]{new} 的方式创建类的对象的。请你运用面向对象的知识，设计 \mil{BluetoothAdapter} 类的 Java 代码的结构。不考虑蓝牙的功能，只考虑该类的其它内部设计，目的是：使得程序员通过 \mil[java]{new} 的方式，不能够创建 \mil{BluetoothAdapter} 类的多个对象。 请思考：该 \mil{BluetoothAdapter} 类应该怎样设计，才能使得程序员无法通过 \mil[java]{new} 的方式创建该类的多个对象？
}
