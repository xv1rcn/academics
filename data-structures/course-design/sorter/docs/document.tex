...
\inputminted{text}{../res/sorted.txt}

该步骤验证三件事：是否成功生成输入文件、四种算法是否全部执行、结果是否分别写入独立文件。

\subsubsection{开启基准并保存 CSV}
...
\inputminted{text}{../res/sorted_benched.txt}

该步骤用于记录耗时与内存净增量，并形成可直接纳入报告的数据文件。

\subsubsection{对输出文件做排序正确性复查}
...
\inputminted{text}{../res/verify.txt}
